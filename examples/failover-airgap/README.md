...
| `ssmJumpS3PrefixListId` | No | `''` | AWS-managed prefix list ID for S3 in this Region (`pl-...`), added as one egress rule on 443 so the jump host can reach the S3 gateway endpoint. Blank means it cannot, which is the default posture. Set it to verify from inside the VPC that the artifacts the BIG-IPs fetch at boot are actually reachable - a bucket check from your workstation exercises a different path. Find it with `aws ec2 describe-managed-prefix-lists --filters Name=prefix-list-name,Values=com.amazonaws.<region>.s3`. |
